...
function checkTeacherLogin() {
  const teacherLoginId = document.getElementById('teacherLoginId').value;
  ...
  google.script.run.withSuccessHandler(function(result) {
    if (result.success) {
      // クライアント側にログインIDを保存
      globalTeacherId = teacherLoginId;
      
      document.getElementById('teacher-login').style.display = 'none';
      document.getElementById('teacher-page').style.display = 'block';
      document.getElementById('teacherName').innerText = `ようこそ、${result.name}さん`;
      document.getElementById('displayedLoginId').innerText = teacherLoginId;
      document.getElementById('displayedPassword').innerText = '*'.repeat(teacherPassword.length);
      populatePromptTable(result.prompts);
      // ログアウトボタン表示
      document.querySelector('#teacher-page .logout-container').style.display = 'block';
      // 利用回数を更新（引数として teacherLoginId を渡す）
      updateUsageCount(teacherLoginId);
      // お知らせ取得
      google.script.run.withSuccessHandler(function(announcementText) {
         console.log("サーバーから返されたお知らせ:", announcementText);
         const announcementTextElement = document.getElementById('announcement-text');
         if (announcementText && announcementText.toString().trim() !== "") {
           announcementTextElement.innerHTML = marked.parse(announcementText);
         } else {
           announcementTextElement.textContent = "現在お知らせはありません。";
         }
      }).getAnnouncement();
    } else {
      document.getElementById('teacherLoginMessage').innerText = result.message || 'ログインに失敗しました。';
    }
    loginButton.innerText = 'ログイン';
    loginButton.disabled = false;
  }).checkTeacherLogin(teacherLoginId, teacherPassword);
}
...
function logout() {
  ...
  document.getElementById('textAnswer').value = ''; // ★テキスト解答欄をリセット
  document.getElementById('problem-text').innerHTML = ''; // ★問題文表示をリセット
  document.getElementById('problem-area').style.display = 'none'; // ★問題文エリアを非表示
  ...
  document.getElementById('newPromptNote').value = '';
  document.getElementById('newPromptText').value = '';
  document.getElementById('newQuestion').value = ''; // ★問題文追加欄をリセット
  document.getElementById('newPromptVisibility').value = '表示';
